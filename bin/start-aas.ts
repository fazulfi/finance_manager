...
import type { AASConfig, Agent, Task } from "@finance/aas";
...
export async function runStartAAS(options: StartAASOptions): Promise<number> {
  ...
  try {
    ...
    let orchestrator!: AASOrchestrator;
    const hooks = createPlanDrivenQualityGateHooks({
      planFilePath: options.planFile,
      unsafeGates,
      source: "start-aas",
      getValidatedPlanFilePath: () => orchestrator.getPlanFilePath(),
    });
    orchestrator = new AASOrchestrator(config, {
